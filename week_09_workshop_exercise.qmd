...
```{python}
## Query the data
total_population = population[population["indicator"] == "SP.POP.TOTL"]
total_population
```

```{python}
## Melt data to long format
population_long = total_population.melt(id_vars=["country","indicator"],var_name="year",value_name ="total_population")
population_long
```
...
hiv_data_combined_pop = pd.merge(hiv_data_combined,population_long,how="left",on=["country_code","year"])
...
```{python}
selected_countries = ["BFA","BDI","KHM","ETH"]
hiv_data_mini = hiv_data_combined_pop[hiv_data_combined_pop["country_code"].isin(selected_countries)]
hiv_data_mini
```
...
fig = px.line(
    ...
    y="incidence_rate",
    color="country_code",
    title="Incidence Rate Over Time by Country",
    labels={
        "year": "Year",
...
fig = px.line(
    ...
    y="prevalence_rate",
    color="country_code",
    title="Prevalence Rate Over Time by Country",
    labels={
        "year": "Year",
        "prevalence_rate": "Incidence Rate",
        "country_code": "Country"
    }
)
fig.show()
